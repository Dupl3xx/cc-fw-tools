...
The combined RFID build repoints all three to a new code-cave string at
`0x00451048`:

```arm
movw r3, #0x1048
movt r3, #0x45     ; r3 = 0x00451048
```
...
## Cave And RFID Compatibility

- `0x00451048` — `0x0045104e` (7 bytes)

The upstream implementation used `0x00450e00`, which lies inside the
`rfid-brand-sync` brand resolver and string table (`0x00450c40` —
`0x00451043`). The relocated address is an aligned, zero-filled gap directly
after the RFID payload, so both patches can be enabled in the same build.
...
All three should show `movw r3, #0x1048` / `movt r3, #0x45`.
